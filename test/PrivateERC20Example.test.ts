...
export const deploymentInfo = { name: "My Confidential Token", symbol: "CTOK", decimals: 2, initialSupply: 500000000n } as const
...
async function expectAllowance(
  ...
  const ctAllowance = await contract["allowance(address,address)"](owner.wallet.address, spenderAddress)
  let allowance = decryptUint(ctAllowance[1], owner.userKey)
...
describe("Confidential ERC20", function () {
  ...
  describe("Deployment", function () {
    ...
    it("Owner initial balance", async function () {
      ...
      const my_CTBalance = await contract["balanceOf(address)"](owner.wallet.address)
